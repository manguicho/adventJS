function canCarry(capacity, trip) {
 // ¡No olvides compartir tu solución en redes!
 	let acc = 0
 	const max = Math.max(...trip.map(e=>Math.max(...e)))
	for (let i=0; i<max; i++){
		for (let o of trip){
			if(o[1] == i) acc+= o[0]
			if(o[2] == i) acc-= o[0]
		}
		if (parseInt(acc) > parseInt(capacity)) return false
	}
return true
}
...
console.log(canCarry(11, [[3, 2, 87], [3, 7, 9], [8, 3, 9]]))
...
console.log(canCarry(2, [[1, 2, 4], [2, 3, 8]])) // false -> del punto 3 al 4 supera la capacidad máxima porque llevaría 3 regalo